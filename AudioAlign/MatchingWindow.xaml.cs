...
namespace AudioAlign
{
    ...
    public partial class MatchingWindow : Window
    {
        ...
        public MatchingWindow(TrackList<AudioTrack> trackList, MultiTrackViewer multiTrackViewer)
        {
            ...
            HaitsmaKalkerFingerprinting = new HaitsmaKalkerFingerprintingViewModel(
                progressMonitor,
                trackList,
                multiTrackViewer.Matches
            );
            WangFingerprinting = new WangFingerprintingViewModel(
                progressMonitor,
                trackList,
                multiTrackViewer.Matches
            );
            EchoprintFingerprinting = new EchoprintFingerprintingViewModel(
                progressMonitor,
                trackList,
                multiTrackViewer.Matches
            );
            ChromaprintFingerprinting = new ChromaprintFingerprintingViewModel(
                progressMonitor,
                trackList,
                multiTrackViewer.Matches
            );
        ...
        private void AlignTracksButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                var message =
                    "Invalid sequence of matches, cannot warp. "
        ...
        private void DriftCalcButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                MessageBox.Show(
                    "Please select two matches between two tracks!",
            ...
            MessageBox.Show(
                String.Format(
                    "Speed of track '{0}' is {2}% compared to track '{1}",
                    m1.Track2.Name,
                    m1.Track2.Name,
                    driftPercentage
                ),
            ...
            if (trackList.Count > 1)
            {
                ...
                if (mode == TimeWarpMode.SelectedTracks)
                {
                    if (trackList.Count > 1)
                    {
                        ...
                        var slaveTracks = new List<AudioTrack>(
                            multiTrackViewer.SelectedItems.Cast<AudioTrack>()
                        );
                        ...
                        foreach (MatchGroup trackGroup in trackGroups)
                        {
                            foreach (MatchPair trackPair in trackGroup.MatchPairs)
                            {
                                List<Match> matches = trackPair
                                    .Matches.OrderBy(match => match.Track1Time)
                                    .ToList();
                                Match first = matches.First();
                                Match last = matches.Last();

                                //Task.Factory.StartNew(() => {
                                TimeSpan sectionLength =
                                    first.Track1Time > first.Track2Time
                                        ? first.Track2Time
                                        : first.Track1Time;
                                TimeWarp(
                                    type,
                                    first.Track1,
                                    first.Track1Time - sectionLength,
                                    first.Track1Time,
                                    first.Track2,
                                    first.Track2Time - sectionLength,
                                    first.Track2Time,
                                    calculateSimilarity,
                                    normalizeSimilarity,
                                    true,
                                ...
                                //Task.Factory.StartNew(() => {
                                sectionLength =
                                    last.Track1.Length - last.Track1Time
                                    > last.Track2.Length - last.Track2Time
                                        ? last.Track2.Length - last.Track2Time
                                        : last.Track1.Length - last.Track1Time;
                                TimeWarp(
                                    type,
                                    last.Track1,
                                    last.Track1Time,
                                    last.Track1Time + sectionLength,
                                    last.Track2,
                                    last.Track2Time,
                                    last.Track2Time + sectionLength,
                                    calculateSimilarity,
                                    normalizeSimilarity,
                                    false,
                                    true
        ...
        )
        {
            ...
            if (TimeWarpDisplay)
            {
                ...
                dtw.OltwInit += new DTW.OltwInitDelegate(
                    delegate(
                        int windowSize,
                ...
                bool drawing = false;
                dtw.OltwProgress += new DTW.OltwProgressDelegate(
                    delegate(int i, int j, int minI, int minJ, bool force)
                    {
                        if (!drawing || force)
                        {
                            dtwPathViewer.Dispatcher.BeginInvoke(
            ...
            if (inOutCue)
            {
                ...
                foreach (Tuple<TimeSpan, TimeSpan> mapping in path)
                {
                    ...
                    if (
                        cueOut
                        && (
                            mapping.Item1 > (t1To - t1From - inOutCueSpan)
                            || mapping.Item2 > (t2To - t2From - inOutCueSpan)
                        )
                    )
                    {
                        endIndex--;
        ...
        private List<MatchGroup> DetermineMatchGroups(MatchFilterMode matchFilterMode)
        {
            ...
            return MatchProcessor.DetermineMatchGroups(
                ...
                trackList,
                matches,
                (bool)windowedModeCheckBox.IsChecked,
                MatchFilterWindowSize
            );
        }
        ...
        private void AddManualMatchPopupComboBoxA_OnSelectionChanged(
            object sender,
            SelectionChangedEventArgs e
        )
        {
            if (
                addManualMatchPopupComboBoxB.Tag as bool? != true // user hasn't done a manual selection in B
                && addManualMatchPopupComboBoxB.Items.Count
                    >= addManualMatchPopupComboBoxA.SelectedIndex
            )
            { // the selected item isn't the last one in the list
                addManualMatchPopupComboBoxB.SelectedIndex =
                    addManualMatchPopupComboBoxA.SelectedIndex + 1; // preset the following item in B
            }
        }

        private void AddManualMatchPopupComboBoxB_OnSelectionChanged(
            object sender,
            SelectionChangedEventArgs e
        )
        {
            if (addManualMatchPopupComboBoxB.IsDropDownOpen)
                    ...
                    Task.Factory.StartNew(() =>
                    {
                        ...
                        )
                        {
                            ...
                            if (
                                ...
                                || t2Interval.TimeTo >= localMP.Track2.Length
                            )
                            {
                            ...
                            computedMatches.Add(
                                new Match
                                {
                                    Track1 = localMP.Track1,
                                    Track1Time =
                                        t1Offset
                                        + position
                                        + (offset < TimeSpan.Zero ? -offset : TimeSpan.Zero),
                                    Track2 = localMP.Track2,
                                    Track2Time =
                                        t2Offset
                                        + position
                                        + (offset >= TimeSpan.Zero ? offset : TimeSpan.Zero),
